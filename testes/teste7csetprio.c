...
int main(int argc, char *argv[]) {
	...
	threadId = ccreate(func0, (void *)&i, 1); // cria uma thread com prioridade 1

    testeFila = FirstFila2(&filaAptoMediaPrioridade);

    if(testeFila == 0){
        thread = (TCB_t *)GetAtIteratorFila2(&filaAptoMediaPrioridade); //Pega primeiro
    }    
